...
import com.jsmosce.adapter.CandidatesHistoryListAdapter;
...
//考生历史记录列表
public class CandidatesHistoryListActivity extends BaseActivity {
    CandidatesHistoryListAdapter CandidatesHistoryListAdapter;
    ...
    public void onEvent(FavorEvent event) {
        ...
        if (event.getId() == FavorEvent.Case_Sorceing) {
            CandidatesHistoryListAdapter.remove(event.getPosition());
        }
    }
    ...
    public void initView() {
        ...
        CandidatesHistoryListAdapter = new CandidatesHistoryListAdapter(getApplicationContext());
        //设置Adapter
        recyclerView.setAdapter(CandidatesHistoryListAdapter);
    ...
    public void getData() {
        ...
        RequestBody body = RequestBody.create(MediaType.parse("application/json; charset=utf-8"), " {\"Pack\":\"Other\",\"Interface\":\"students\",\"caseId\":" + BaseApplication.caseId + "}");
        ...
        call.enqueue(new Callback<DataStudentsList>() {
            ...
            public void onResponse(Call<DataStudentsList> call,
                ...
                switch (dataHomeTest.getState()) {
                    case 1:
                        //添加数据
                        CandidatesHistoryListAdapter.upData(dataHomeTest.getInfo().getUserInfo(), dataHomeTest.getInfo().getCaseInfo());
